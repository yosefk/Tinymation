...
extern "C" int fitpack_parcur(const double* points, const double* weights, int points_dimension, int num_points, int spline_degree,
                              double smoothing_factor, double* knots, int* num_knots, double* coefficients)
{
    ...
    double ub = 0;
    double ue = 0;
    int nest = num_points + spline_degree + 1;
    ...
    parcur_(&iopt, &ipar, &points_dimension, &num_points, &u[0], &mx, const_cast<double*>(points), const_cast<double*>(weights),
